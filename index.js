...
// ---------- Triển khai server (giúp chạy trang localhost:8000)---------
...
setInterval(
    function () {
        ...
        con.query(sql_turbine1, function (err, result) {
            if (err) throw err
            console.log("Data selected");
            result.forEach(function (value1) {
                console.log(value1)
                windSpeed1 = value1.TocDoGio + Math.floor(Math.random()*2); 
                windPower1 = value1.CongSuat + Math.floor(Math.random()*100); 
        ...
        windPower = windPower1+windPower2+windPower3+windPower4+windPower5+windPower6+windPower7+windPower8+windPower9+windPower10+windPower11+windPower12;
        io.on("connection", () => {
            io.emit('server-update-data1', { windSpeed1, windPower1, windDirection1,windStatus1 })
            // console.log(windSpeed1 + " + " + windPower1 + " + " + windDirection1 + " + " + windStatus1);
